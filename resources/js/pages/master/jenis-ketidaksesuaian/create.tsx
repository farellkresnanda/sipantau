...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Create Jenis Ketidaksesuaian', href: '/master/jenis-ketidaksesuaian/create' },
];

// ✅ sub_jenis tetap array, tapi boleh kosong
const formSchema = z.object({
  nama: z.string().min(1, { message: 'Nama Jenis harus diisi' }).max(255),
  sub_jenis: z
    .array(
      z.object({
        nama: z.string().optional(), // boleh kosong
      })
    ),
});

export default function CreateJenisKetidaksesuaian() {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    const filtered = {
      ...values,
      sub_jenis: values.sub_jenis.filter((sj) => sj.nama && sj.nama.trim() !== ''),
    };

    router.post('/master/jenis-ketidaksesuaian', filtered, {
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="Create Jenis Ketidaksesuaian" />
      <div className="space-y-6 p-4">
        <div className="flex items-center justify-between">
          <SectionHeader
            title="Buat Jenis Ketidaksesuaian"
            subtitle="Tambahkan jenis ketidaksesuaian baru beserta sub-jenisnya (opsional)"
          />
        </div>
