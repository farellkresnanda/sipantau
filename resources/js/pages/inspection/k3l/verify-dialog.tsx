...
interface ValidatorVerifyPpeDialogProps {
    inspection: {
        uuid: string;
...
export default function ValidatorVerifyPpeDialog({ inspection }: ValidatorVerifyPpeDialogProps) {
    const [open, setOpen] = useState(false);
    const { auth } = usePage<SharedData>().props;
    const role = auth.role;

    // Hanya Validator yang bisa lihat tombol
    ...
    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        router.post(`/inspection/k3l/verify/${inspection.uuid}`, finalFormData, {
            preserveScroll: true,
            forceFormData: true,
            onSuccess: () => {
                setOpen(false);
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Verifikasi Inspeksi K3L</DialogTitle>
                    <DialogDescription>Form ini hanya untuk Validator memverifikasi hasil inspeksi K3L.</DialogDescription>
                </DialogHeader>
